...
public class WaveOption : MonoBehaviour
{
    ...
    {
        EventManager.TriggerEvent("OnWaterPush");
    ...
    void ReleaseWave(Vector3 point)
    {
        ...
        EventManager.TriggerEvent("OnWaterRelease");
        if (Origin != Vector3.zero)
        {
            LevelManager.StartTimer();
            GameObject generator = Instantiate(WavePrefab, point, Quaternion.identity) as GameObject;
            generator.GetComponent<PathWaveGenerator>().SetDireciton(new Vector2(Origin.x - point.x, Origin.y - point.y));
            Origin = Vector3.zero;
